  ...
  return (
    <EditableSiteShell>
      <main style={archiveVars} className="bg-[var(--archive-bg)] text-[var(--archive-text)]">
        <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-16">
          <div className="border border-[var(--editable-border)] bg-[var(--archive-surface)] p-7 sm:p-10">
            <div className="inline-flex items-center gap-2 border border-[var(--editable-border)] bg-white/60 px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-[var(--archive-accent)]"><Icon className="h-4 w-4" /> {label}</div>
            <h1 className="editable-serif mt-5 max-w-4xl text-5xl font-normal leading-[0.98] text-[var(--archive-accent)] sm:text-6xl">{voice?.headline || `Browse ${label}`}</h1>
            <p className="mt-6 max-w-2xl text-base leading-8 opacity-70">{voice?.description || SITE_CONFIG.description}</p>
            <div className="mt-6 border-l border-[var(--editable-border)] bg-white/35 p-4 text-sm font-bold leading-7 opacity-75">{deck.promise}</div>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={basePath} className="bg-[var(--archive-text)] px-5 py-3 text-sm font-black text-[var(--archive-bg)]">Browse all</Link>
              <Link href="/search" className="border border-[var(--editable-border)] px-5 py-3 text-sm font-black">Search posts</Link>
            </div>
          </div>

          <form action={basePath} className="self-end border border-[var(--editable-border)] bg-white/50 p-5 backdrop-blur">
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-8">
          {posts.length ? (
            <div className={task === 'article' ? 'editable-horizontal-strip px-0 pb-6' : deck.archiveClass}>
  ...
  return (
    <Link href={href} className="group block w-[min(78vw,390px)] shrink-0 border-r border-[var(--editable-border)] bg-[var(--archive-surface)] p-5 transition hover:bg-white/70">
      ...
      <div className="pt-5">
        ...
        <h2 className="editable-serif mt-2 text-3xl font-normal leading-tight text-[var(--archive-accent)]">{post.title}</h2>
